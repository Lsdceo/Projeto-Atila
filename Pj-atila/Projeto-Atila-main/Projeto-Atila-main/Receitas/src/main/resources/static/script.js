...
async function addRecipe(title, description, url) {
    try {
        const response = await fetch(`http://localhost:8080/recipes`, {
            ...
            body: JSON.stringify({ titles: title, description, url }),
        });
        ...
        renderRecipes();
        showFeedbackMessage('Receita adicionada com sucesso!');
    } catch (error) {
        console.error('Erro ao adicionar receita:', error);
        showFeedbackMessage('Erro ao adicionar receita. Tente novamente.');
    }
}
...
async function updateRecipe(recipeId, titles, description, url) {
    try {
